    ...
    <div>
      ...
      {editing ? (
        <input autoFocus type={type} value={val} onChange={e => setVal(e.target.value)} onBlur={handleBlur} onKeyDown={e => { if (e.key === 'Enter') handleBlur() }}
          className="w-full px-2 py-1 text-sm rounded-md outline-none" style={{ background: '#EEF7F2', border: '1px solid #1aaa5e', color: '#191D25' }} />
      ) : (
        <div className="text-sm py-1 px-2 rounded-md cursor-text hover:bg-white/5 transition-colors min-h-[28px]" style={{ color: val ? '#191D25' : '#8aaa98' }} onClick={() => setEditing(true)}>
  ...
  return (
    <div className="flex flex-col h-full">
      <TopBar title={`${contact.first_name} ${contact.last_name}`} breadcrumb={[{ label: 'Contacts', href: '/contacts' }, { label: `${contact.first_name} ${contact.last_name}` }]}
        action={<button onClick={() => router.push('/contacts')} className="flex items-center gap-1.5 text-sm px-3 py-2 rounded-md hover:bg-white/5" style={{ color: '#638070' }}><ArrowLeft size={14} /> Back</button>} />
      <div className="flex-1 overflow-y-auto p-6">
        <div className="flex gap-6 max-w-6xl">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-12 h-12 rounded-full flex items-center justify-center text-lg font-semibold" style={{ background: '#EEF7F2', color: '#2cc774' }}>
                {contact.first_name.charAt(0)}
              </div>
              <div>
                <h1 className="text-2xl font-semibold" style={{ color: '#191D25' }}>{contact.first_name} {contact.last_name}</h1>
                <p className="text-sm" style={{ color: '#638070' }}>{contact.job_title ?? ''}{contact.job_title && (contact as any).company ? ' at ' : ''}{(contact as any).company?.name ?? ''}</p>
              </div>
            </div>
            ...
            {tab === 'overview' && (
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="col-span-2">
                  <div className="text-xs font-medium mb-1" style={{ color: '#8aaa98' }}>Notes</div>
                  <textarea className="w-full px-2 py-1.5 text-sm rounded-md outline-none resize-none" style={{ background: 'transparent', border: '1px solid transparent', color: '#191D25' }}
            ...
            {tab === 'deals' && (
              deals.length === 0 ? <p className="text-sm" style={{ color: '#8aaa98' }}>No deals linked</p> : (
                <div className="rounded-xl overflow-hidden" style={{ border: '1px solid #D4E8DC' }}>
                  <table className="w-full border-collapse">
                    <thead style={{ background: '#F8FBF9' }}>
                      <tr>{['Deal', 'Value', 'Stage', 'Close Date'].map(h => <th key={h} className="text-left px-4 py-2.5" style={{ color: '#8aaa98', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '1px solid #D4E8DC' }}>{h}</th>)}</tr>
                    </thead>
            ...
            <div className="rounded-xl p-4 space-y-3" style={{ background: '#F8FBF9', border: '1px solid #D4E8DC' }}>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <select value={contact.status} onChange={e => updateField('status', e.target.value)} className="text-xs rounded px-2 py-1 outline-none" style={{ background: '#EEF7F2', border: '1px solid #D4E8DC', color: '#191D25' }}>
