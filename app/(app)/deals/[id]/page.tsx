  ...
  return (
    <div className="flex flex-col h-full">
      <TopBar title={deal.name} breadcrumb={[{ label: 'Deals', href: '/deals' }, { label: deal.name }]}
        action={<button onClick={() => router.push('/deals')} className="flex items-center gap-1.5 text-sm px-3 py-2 rounded-md hover:bg-white/5" style={{ color: '#9090A8' }}><ArrowLeft size={14} /> Back</button>} />
      <div className="flex-1 overflow-y-auto p-6">
        <div className="flex gap-6 max-w-6xl">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-4">
              <div>
                <h1 className="text-2xl font-semibold" style={{ color: '#F4F4F8' }}>{deal.name}</h1>
            ...
            {tab === 'overview' && (
              <div className="space-y-5">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <div className="text-xs font-medium mb-1" style={{ color: '#5A5A70' }}>Value (€)</div>
                    <input type="number" defaultValue={deal.value} className="w-full px-2 py-1 text-sm rounded-md outline-none"
                      ...
                      onBlur={e => { e.target.style.borderColor = '#2A2A38'; updateField('value', parseFloat(e.target.value) || 0) }} />
                  </div>
                  <div>
                    <div className="text-xs font-medium mb-1" style={{ color: '#5A5A70' }}>Stage</div>
                    <select value={deal.stage} onChange={e => updateField('stage', e.target.value)} className="w-full px-2 py-1 text-sm rounded-md outline-none" style={{ background: '#111118', border: '1px solid #2A2A38', color: '#F4F4F8' }}>
                  ...
                  <div>
                    <div className="text-xs font-medium mb-2" style={{ color: '#5A5A70' }}>Probability: <span style={{ color: '#6366F1' }}>{deal.probability}%</span></div>
                  ...
                  <div>
                    <div className="text-xs font-medium mb-1" style={{ color: '#5A5A70' }}>Close Date</div>
                    <input type="date" defaultValue={deal.close_date ?? ''} className="w-full px-2 py-1 text-sm rounded-md outline-none" style={{ background: '#111118', border: '1px solid #2A2A38', color: '#F4F4F8', colorScheme: 'dark' }}
                      onBlur={e => updateField('close_date', e.target.value || null)} />
                  </div>
                </div>
                {(deal as any).company && (
                  <div>
                    <div className="text-xs font-medium mb-1" style={{ color: '#5A5A70' }}>Company</div>
                    <button onClick={() => router.push(`/companies/${deal.company_id}`)} className="text-sm hover:underline" style={{ color: '#6366F1' }}>{(deal as any).company.name}</button>
                  </div>
                )}
                {(deal as any).contact && (
                  <div>
                    <div className="text-xs font-medium mb-1" style={{ color: '#5A5A70' }}>Contact</div>
                    <button onClick={() => router.push(`/contacts/${deal.contact_id}`)} className="text-sm hover:underline" style={{ color: '#6366F1' }}>{(deal as any).contact.first_name} {(deal as any).contact.last_name}</button>
                  </div>
                )}
                <div>
            ...
            <div className="rounded-xl p-4 space-y-3" style={{ background: '#111118', border: '1px solid #2A2A38' }}>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs" style={{ color: '#9090A8' }}>Stage</span>
                  <StatusBadge type="deal" value={deal.stage} />
                </div>
                <div className="flex justify-between">
                  <span className="text-xs" style={{ color: '#9090A8' }}>Probability</span>
                  <span className="text-xs" style={{ color: '#6366F1' }}>{deal.probability}%</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs" style={{ color: '#9090A8' }}>Created</span>
                  <span className="text-xs" style={{ color: '#F4F4F8' }}>{formatDate(deal.created_at)}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-xs" style={{ color: '#9090A8' }}>Updated</span>
                  <span className="text-xs" style={{ color: '#F4F4F8' }}>{formatDate(deal.updated_at)}</span>
                </div>
              </div>
            </div>
          </div>
